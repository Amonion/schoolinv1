      ...
      <div class="admin-plan-form-flex">
        <div class="w-layout-grid grid-form">
          <div class="admin-input-wrapper">
            <label for="name-5" class="admin-input-label">Faculty </label
            ><input
              type="text"
          ...
          <!-- <div class="admin-input-wrapper">
            <label for="name-6" class="admin-input-label">Course </label>
            <div class="admin-text-input" @click="showCourses = !showCourses">
              <div>{{ course }}</div>
              <img
                src="https://assets-global.website-files.com/656ace5627dc5da51719088f/656b74de84a7b6a965eb8630_caret-down.svg"
                loading="lazy"
          ...
          <div class="admin-input-wrapper">
            <label for="name-5" class="admin-input-label">Faculty Video </label
            ><input
              type="text"
              class="admin-text-input w-input"
              v-model="videoUrl"
...
export default {
  ...
  methods: {
    ...
    async getFaculties() {
      await this.$store.dispatch(
        "admin/getFaculties",
        `/faculties/?limit=${this.limit}&page=${this.currentPage}`
      );
    },
    ...
    async processData() {
      ...
      const url = this.isEditing
        ? `/faculties/?id=${this.editingId}&limit=${this.limit}&page=${this.currentPage}`
        : `/faculties/?limit=${this.limit}&page=${this.currentPage}`;
    ...
    resultResponse(result) {
      ...
      if (!result.response) {
        this.$store.commit("admin/setFaculties", result.data);
        this.clearInputs();
      } else {
        this.showModal();
      }
  ...
  computed: {
    totalLength() {
      return this.$store.state.admin.facultyLength;
    },

    selectedItems() {
      return this.$store.state.admin.selectedFaculties;
    },

    isAllItemsSelected() {
      return this.$store.state.admin.isAllFacultiesSelected;
    },
