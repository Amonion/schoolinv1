  ...
  <div class="admin-container">
    <div class="admin-home-flex">
        ...
        <div class="admin-users-scroll">
          <div class="admin-users-tbs">
            <div class="admin-home-table-head">
              ...
              <div class="admin-tl-time1"><div>School</div></div>
              <div class="admin-tl-time1"><div>Faculty</div></div>
              <div class="admin-tl-time1"><div>Department</div></div>
            ...
            >
              ...
              <div class="admin-tl-name1">
                <NuxtLink
                  :to="`/admin/user-details/?username=${item.username}&id=${item.id}`"
              ...
              <div class="admin-tl-time1 flex flex-col">
                <div>
                  {{ formatNumberToDate(item.dateCreated * 1) }} <br />{{
                    formatDateToTime(item.dateCreated * 1)
                  }}
                </div>
                <div></div>
              </div>
      ...
      <div class="admin-table-action">
        ...
        <div class="admin-action-items">
          <div class="admin-action-drop-head" @click="makeStaff">
            <div>Make Staff</div>
          </div>
        </div>
        <img
  ...
  methods: {
    ...
    async getUsers() {
      await this.$store.dispatch(
        "admin/getUsers",
        `/users/?sort=${this.sort}&limit=${this.limit}&page=${this.currentPage}&status=${this.status}`
      );
    },
    ...
    async makeStaff() {
      ...
      const payload = {
        url: `/users/toggle-status/?sort=${this.sort}&limit=${this.limit}&page=${this.currentPage}&status=${this.status}`,
        form: this.selectedUsers,
      };
    ...
    handleResponse(result) {
      if (!result.response) {
        this.$store.commit("admin/setUsers", result.data);
      } else {
    ...
    prepareDelete(id) {
      const payload = {
        ...
        table: "users",
        action: "delete",
        url: `/users/?id=${id}&limit=${this.limit}&page=${this.currentPage}&status=${this.status}`,
        item: "",
      };
    ...
    prepareSuspend(item) {
      ...
      this.showModal(
        ...
        false,
        "update",
        `/users/update-user/?id=${item.id}&limit=${this.limit}&page=${this.currentPage}&status=${this.status}`,
        form
      );
    },
